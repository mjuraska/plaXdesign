...
#'
#' Considering a two-arm vaccine efficacy trial with allocation ratio 1:1 and two periods of participant follow-up with associated levels of vaccine efficacy \eqn{VE_1} and \eqn{VE_2}, \code{plaXpower} computes for both a placebo crossover and standard non-crossover design (i) power to reject the null hypothesis \eqn{H_0: VE_1 = VE_2} in favor of the one-sided alternative hypothesis of waning VE, i.e., \eqn{H_1: VE_1 > VE_2}, (ii) power to reject the null hypothesis \eqn{H_0: VE_2 \ge 0} in favor of the one-sided alternative hypothesis of vaccine harm, i.e., \eqn{H_1: VE_2 < 0}, and (iii) for each hypothesis test, the sample size ratio (placebo crossover/standard) required to achieve the same power. The event of interest is assumed to be rare, with the event count following a Poisson distribution in each arm and time period. In each arm, the event counts in different time periods are assumed to be independent. The vaccine administered in period 2 is assumed to have equal VE as in period 1.
#'
#' @aliases print.plaXpower
#' @param theta1 either a scalar or a numeric vector (same choice is required for both \code{theta1} and \code{theta2}) specifying the Poisson mean event count(s) in the original placebo arm in period 1 in the standard design. If a scalar is specified, the same mean event count in period 1 is assumed in all scenarios described by \code{ve1} and \code{ve2}. If a vector, it must be of the same length as \code{ve1} and \code{ve2}, with matching components treated as a single trial scenario.
#' @param theta2 either a scalar or a numeric vector specifying the Poisson mean event count in the original placebo arm in period 2 in the standard design. If a scalar is specified, the same mean event count in period 2 is assumed in all scenarios described by \code{ve1} and \code{ve2}. If a vector, it must be of the same length as \code{ve1} and \code{ve2}, with matching components treated as a single trial scenario.
...
#' \item \code{powerHarmX}: power to detect vaccine harm in period 2 in the placebo crossover design
#' \item \code{powerHarmStd}: power to detect vaccine harm in period 2 in the standard non-crossover design
...
#' \item \code{ssRatioHarmXstd}: the sample size ratio (placebo crossover/standard) that ensures the same power to detect vaccine harm in period 2
#' }
#'
#' @examples
#' # analytical method
...
plaXpower <- function(theta1, theta2, ve1, ve2, method=c("analytical", "simulation"), alpha=0.025, iter=1e3, seed=NULL){
  ...
  if (method=="analytical"){
    ...
    out <- data.frame(out, powerWaneX, powerWaneStd, powerHarmX, powerHarmStd,
                      ssRatioWaneXstd=varRatioWaneXstd, ssRatioHarmXstd=varRatioHarmXstd)
    ...
    out2 <- lapply(1:length(ve1), function(i, theta1, theta2, ve1, ve2, iter, qZ){
      ...
      return(c(powerWaneX, powerWaneStd, powerHarmX, powerHarmStd, meanVarRatioWaneXstd, meanVarRatioHarmXstd))
    }, theta1=theta1, theta2=theta2, ve1=ve1, ve2=ve2, iter=iter, qZ=qZ)

    out <- data.frame(out, do.call(rbind, out2))
    colnames(out)[-(1:4)] <- c("powerWaneX", "powerWaneStd", "powerHarmX", "powerHarmStd", "ssRatioWaneXstd", "ssRatioHarmXstd")
  }
